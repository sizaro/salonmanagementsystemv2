...
export async function getReportData({
  ...
  const [
    ...
    db.query(
      `SELECT * FROM expenses WHERE salon_id = $3 AND (created_at AT TIME ZONE $4)::date BETWEEN $1::date AND $2::date ORDER BY created_at DESC`,
      [startDate, endDate, salonId, timezone],
    ),
    db.query(
      `SELECT a.*, u.first_name, u.last_name FROM advances a LEFT JOIN users u ON u.id = a.employee_id AND u.salon_id = $3 WHERE a.salon_id = $3 AND a.advance_date BETWEEN $1::date AND $2::date ORDER BY a.advance_date DESC, a.advance_time DESC`,
    ...
    db.query(
      `SELECT tf.*, CONCAT(u.first_name, ' ', u.last_name) AS employee_name FROM tag_fee tf LEFT JOIN users u ON u.id = tf.employee_id AND u.salon_id = $3 WHERE tf.salon_id = $3 AND (tf.created_at AT TIME ZONE $4)::date BETWEEN $1::date AND $2::date ORDER BY tf.created_at DESC`,
      [startDate, endDate, salonId, timezone],
    ),
    db.query(
      `SELECT lf.*, CONCAT(u.first_name, ' ', u.last_name) AS employee_name FROM late_fees lf LEFT JOIN users u ON u.id = lf.employee_id AND u.salon_id = $3 WHERE lf.salon_id = $3 AND (lf.created_at AT TIME ZONE $4)::date BETWEEN $1::date AND $2::date ORDER BY lf.created_at DESC`,
      [startDate, endDate, salonId, timezone],
    ),
    db.query(
      `SELECT id, salon_id, status, open_date::text AS open_date, open_time::text AS open_time, close_date::text AS close_date, close_time::text AS close_time, created_at, updated_at FROM salon_sessions WHERE salon_id = $3 AND open_date BETWEEN $1::date AND $2::date ORDER BY open_date DESC, open_time DESC`,
      [startDate, endDate, salonId],
    ),
    db.query(
      `SELECT u.*, (u.created_at AT TIME ZONE $2) AS employee_time FROM users u WHERE u.salon_id = $1 AND u.role IN ('employee', 'manager', 'owner') ORDER BY u.id ASC`,
      [salonId, timezone],
    ),
  ]);
  return {
